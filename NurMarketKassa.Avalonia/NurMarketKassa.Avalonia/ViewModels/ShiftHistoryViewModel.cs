...
public sealed class ShiftHistoryViewModel : INotifyPropertyChanged
{
    ...
    public ShiftHistoryViewModel(Window owner)
    {
        ...
        OpenShiftActionsCommand = new AsyncRelayCommand<ShiftModel>(OpenShiftActionsAsync);
    ...
    public ShiftPage ActivePage
    {
        ...
        set
        {
            ...
            {
                ApplySummaryFilters();
                RecalculateSummaryFromOperations(_allFilteredOperations, openingBalance: SumOpeningCashForFilteredShifts());
            }
            else
                RecalculateSummary();
        }
    ...
    /// <summary>Фактически пересчитанная касса при закрытии смены. null — смена ещё не закрыта.</summary>
    public decimal? ActualBalance { get; private set; }
    public decimal? Difference { get; private set; }
    public decimal RevenueTotal { get; private set; }
    public bool HasActualBalance => ActualBalance.HasValue;
    ...
    public string ActualBalanceText => ActualBalance is { } actual ? FormatMoney(actual) : "—";
    public string DifferenceText => Difference is { } diff ? FormatSignedMoney(diff) : "—";
    ...
    private void RecalculateSummary()
    {
        ...
        RecalculateSummaryFromOperations(
            ops, shiftRevenue: RevenueTotal, actualBalance: shift?.ClosingCash, openingBalance: shift?.OpeningCash);
    }

    private void RecalculateSummaryFromOperations(
        IEnumerable<CashOperationModel> ops,
        decimal? shiftRevenue = null,
        decimal? actualBalance = null,
        decimal? openingBalance = null)
    {
        ...
        // 2026-09-13, живой баг: раньше начальная сумма считалась из локальных операций с типом
        // OpeningBalance, которые нигде в коде не создаются (реальная стартовая сумма смены
        // пишется только на сервер, см. ShiftModel.OpeningCash) — "Начальная сумма" всегда
        // показывала 0, занижая "Ожидаемую сумму" ровно на стартовую наличность каждой смены.
        // Вызывающий код теперь передаёт настоящую сумму с сервера явным параметром; фоллбэк на
        // операции оставлен как безопасный no-op на случай, если он когда-нибудь понадобится.
        OpeningBalance = openingBalance ?? list.Where(o => o.Kind == CashOperationKind.OpeningBalance).Sum(o => o.Amount);
        ...
        // Фактический остаток — это ТОЛЬКО пересчитанная кассиром сумма в ящике при закрытии смены.
        // Пересчёт его из тех же операций, что и ожидаемый баланс, давал бы Difference == -RevenueTotal всегда.
        ActualBalance = actualBalance;
        Difference = ActualBalance is { } actual ? actual - ExpectedBalance : null;
    ...
    private void NotifySummaryProperties()
    {
        ...
        OnPropertyChanged(nameof(HasActualBalance));
    }

    /// <summary>Сумма стартовых сумм всех смен, попадающих под те же фильтры даты/кассира, что
    /// и _allFilteredOperations ниже — для карточки "Общий план" на несколько смен сразу (см.
    /// комментарий у OpeningBalance в RecalculateSummaryFromOperations).</summary>
    private decimal SumOpeningCashForFilteredShifts()
    {
        var query = AllShifts.AsEnumerable();
        if (FilterDateFrom is { } from) query = query.Where(s => (s.OpenedAt ?? DateTime.MinValue).Date >= from.Date);
        if (FilterDateTo is { } to) query = query.Where(s => (s.OpenedAt ?? DateTime.MinValue).Date <= to.Date);
        if (!string.IsNullOrWhiteSpace(FilterCashier) && FilterCashier != "Все кассиры")
            query = query.Where(s => string.Equals(s.Cashier, FilterCashier, StringComparison.OrdinalIgnoreCase));
        return query.Sum(s => s.OpeningCash ?? 0m);
    }
    ...
    private void OpenNewOperation()
    {
        ...
        if (PosDialogHost.Show(dialog, _owner) == true && dialog.ResultOperation is { } op)
        {
            ...
            // Изъятие — это расход смены. Внесение расходом не считается, поэтому пишем только
            // отрицательные операции (см. ShiftEventsStore).
            if (op.Amount < 0m)
            {
                ShiftEventsStore.Record(
                    ShiftEventsStore.KindExpense, PosApp.ActiveShiftId, op.Id, (double)Math.Abs(op.Amount), op.Comment);
            }
            ReloadOperations();
            RecalculateSummary();
            ApplySummaryFilters();
    ...
    private async Task OpenShiftActionsAsync(ShiftModel? shift)
    {
        ...
        switch (action)
        {
            case ShiftMenuAction.View:
                await ShowShiftDetailsWithResolvedDebtAsync(shift).ConfigureAwait(true);
                break;
            case ShiftMenuAction.Print:
                // 2026-09-14, по просьбе пользователя ("нужен подробный отчёт... как на
                // вебке") — раньше здесь была заглушка "будет доступно в следующем
                // обновлении". ShiftDetailsDialog теперь сам умеет печатать (см. её
                // Print_Click), поэтому просто переиспользуем тот же диалог, что и "Просмотр".
                await ShowShiftDetailsWithResolvedDebtAsync(shift).ConfigureAwait(true);
                break;
            case ShiftMenuAction.Deposit:
            case ShiftMenuAction.Withdraw:
                // 2026-09-13, живой баг: Внесение/Изъятие отсюда всегда писались в ТЕКУЩУЮ
                // открытую смену (ShiftCashOperationsStore.Append берёт PosApp.ActiveShiftId,
                // а не смену, на чьей строке нажали в Истории) — операция по старой закрытой
                // смене задваивала баланс текущей открытой, а если открытой смены не было
                // вообще, сумма пропадала бесследно (NetForShift игнорирует ShiftId=null).
                // Эти операции физически имеют смысл только для смены, которая прямо сейчас
                // открыта — для любой другой блокируем с понятным объяснением вместо тихой
                // порчи чужого баланса.
                if (!shift.IsActive)
                {
    ...
    /// не то") — shift.DebtSales здесь пришёл из непроверенного поля списка смен сервера (см.
    /// ShiftHistoryService.Parse), а не из того же надёжного расчёта, что уже работает сразу
    /// после закрытия смены. Показываем диалог сразу со старым значением (не блокируем клик),
    /// затем уточняем долг тем же способом, что и CashShiftService, и обновляем диалог, если
    /// он ещё открыт.</summary>
    private async Task ShowShiftDetailsWithResolvedDebtAsync(ShiftModel shift)
    {
        var dialog = new ShiftDetailsDialog(shift);
        var dialogTask = PosDialogHost.ShowAsync(dialog, _owner);

        try
        {
            var cashShift = App.GetRequiredService<ICashShiftService>();
            var resolvedDebt = await cashShift
        ...
        {
            PosLogger.Log($"ShiftHistory: resolve shift debt failed for {shift.Id}: {ex.GetType().Name}", "SHIFTS");
        }
