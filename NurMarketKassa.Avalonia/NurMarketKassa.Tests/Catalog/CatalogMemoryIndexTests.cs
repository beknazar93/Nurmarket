...
public sealed class CatalogMemoryIndexTests : IDisposable
{
    ...
    /// <summary>2026-09-12: "программа не видит доп. штрихкод" — "Дополнительные штрихкоды" из
    /// карточки товара сохранялись (AlternateBarcodesRaw), но BuildCache индексировал только
    /// основной Barcode, так что ни поиск на кассе, ни в Складе никогда их не находил.</summary>
    [Fact]
    public void TryGetTileByBarcode_FindsProduct_ByAlternateBarcode()
    {
        var milk = CatalogTestHelper.CreateTile("milk-1", "Молоко 3.2%", "4601234567890", 120);
        milk.AlternateBarcodesRaw = "4600000000017\n4600000000024, 4600000000031";
        _repository.SyncReplaceAllWithDiff([milk]);

        _repository.TryGetTileByBarcode("4600000000017")!.Id.Should().Be("milk-1", "первый доп. код на своей строке");
        _repository.TryGetTileByBarcode("4600000000024")!.Id.Should().Be("milk-1", "коды через запятую тоже должны разбираться");
        _repository.TryGetTileByBarcode("4600000000031")!.Id.Should().Be("milk-1");
        _repository.TryGetTileByBarcode("4601234567890")!.Id.Should().Be("milk-1", "основной штрихкод продолжает работать как раньше");
    }

    [Fact]
    public void TryGetTileByBarcode_PrimaryBarcode_WinsOverConflictingAlternate()
    {
        var milk = CatalogTestHelper.CreateTile("milk-1", "Молоко 3.2%", "4601234567890", 120);
        var bread = CatalogTestHelper.CreateTile("bread-1", "Хлеб белый", "4609876543210", 50);
        // Опечатка кассира: доп. код у хлеба случайно совпал с основным штрихкодом молока —
        // основной штрихкод не должен перекрываться чужим доп. кодом.
        bread.AlternateBarcodesRaw = "4601234567890";
        _repository.SyncReplaceAllWithDiff([milk, bread]);

        _repository.TryGetTileByBarcode("4601234567890")!.Id.Should().Be("milk-1");
    }
