...
public sealed class UserSession
{
    ...
    /// <summary>2026-09-09: когда сессия последний раз подтверждалась реальным успешным
    /// онлайн-обращением к серверу (не просто "токен ещё не истёк по времени" — ExpiresAt
    /// выше это уже покрывает). Null у сессий, сохранённых до этого поля — трактуется как
    /// "не проверяем" (см. OnlineOfflineAuthenticationService.IsWithinOfflineGracePeriod), чтобы
    /// обновление кассы не обрывало уже идущую офлайн-работу задним числом.</summary>
    public DateTimeOffset? LastOnlineContactAt { get; init; }
}
