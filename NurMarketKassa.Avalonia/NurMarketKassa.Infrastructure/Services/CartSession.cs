...
public sealed class CartSession : IDisposable
{
    ...
    /// <summary>
    /// Same as <see cref="SetCart"/> but takes ownership of an already-parsed
    /// document instead of re-parsing its raw text — the caller must not
    /// dispose <paramref name="document"/> afterwards. Large receipts mutate
    /// through a full JSON round trip on every single line change, so avoiding
    /// this second parse matters once a basket grows into the hundreds of items.
    /// </summary>
    internal void SetCartFromOwnedDocument(JsonDocument document)
    {
        ApplyRootMetadata(document.RootElement);
        _doc?.Dispose();
        _doc = document;
    }

    private void ApplyRootMetadata(JsonElement root)
    {
