    ...
    private async Task<bool> RefreshAccessUnlockedAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            if (root.TryGetProperty("access", out var acc) && acc.ValueKind == JsonValueKind.String)
            {
                ...
                // 2026-09-21, живой баг ("Сессия недействительна" при большом каталоге):
                // сервер ротирует refresh-токен при каждом успешном /auth/refresh/ (отдаёт новый
                // "refresh" в ответе, старый после этого недействителен на сервере) — этот метод
                // читал только "access" и НИКОГДА не обновлял Session.RefreshToken. При большом
                // каталоге запросы идут долго и параллельно (см. Session.HttpSlots), поэтому за
                // одну синхронизацию access-токен успевает истечь и обновиться НЕСКОЛЬКО раз;
                // второй такой цикл пытался обновиться по уже устаревшему (отозванному сервером)
                // refresh-токену, получал отказ, и код ниже вызывал Session.Clear() — кассир видел
                // "Сессия недействительна", хотя на самом деле никто не заходил с другого ПК/телефона.
                // См. эталонную реализацию в NurMarketApiClient.RefreshAccessUnlockedAsync.
