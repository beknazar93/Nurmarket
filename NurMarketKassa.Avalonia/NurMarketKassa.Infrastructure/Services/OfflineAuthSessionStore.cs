...
public static class OfflineAuthSessionStore
{
    ...
    public static OfflineAuthSession? TryLoad()
    {
        lock (SyncRoot)
        {
            ...
            {
                PosLogger.Log($"Legacy auth session load failed: {ex.GetType().Name}", "WARNING");
                return null;
            }
        }
    }
    ...
    /// <summary>2026-09-09: жёсткий потолок офлайн-работы для ОБЫЧНОГО (NurCRM) режима — 60
    /// часов подряд без единого успешного онлайн-контакта (LastAuthAt обновляется и при логине,
    /// и при каждом успешном обновлении токена — см. UpdateTokens). Раньше проверки по времени
    /// не было вообще, офлайн-сессия была юзабельна бессрочно. Ограничение НЕ касается
    /// активированного автономного режима (см. LocalAuthApiService) — тот работает офлайн без
    /// предела, это его смысл; здесь речь только про "временное" офлайн-продолжение обычного
    /// онлайн-аккаунта.</summary>
    public static readonly TimeSpan MaxOfflineDuration = TimeSpan.FromHours(60);
    ...
    public static bool IsUsable(OfflineAuthSession? session) =>
        ...
        && DateTimeOffset.Now - session.LastAuthAt <= MaxOfflineDuration;
    ...
    public static void Clear()
    {
        lock (SyncRoot)
        {
            ...
            {
                PosLogger.Log($"Legacy auth session cleanup failed: {ex.GetType().Name}", "WARNING");
            }
        }
    }
    ...
    private static void ExtractUserFields(JsonElement user, OfflineAuthSession session)
    {
        ...
        session.Permissions = PermissionService.ExtractPermissionNames(user).ToList();
    }
    ...
    private static void TryDeleteLegacyJson()
    {
        ...
        {
            PosLogger.Log($"Legacy auth JSON cleanup failed: {ex.GetType().Name}", "WARNING");
        }
    }
}
